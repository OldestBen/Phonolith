...
function streamUrl(hash: string): string {
  return `/stream/${hash}`
}

async function fetchAndDecode(ctx: AudioContext, hash: string): Promise<AudioBuffer> {
  const res = await fetch(streamUrl(hash))
...
export function useGaplessPlayer(): GaplessPlayer {
  ...
  const describeSignalPath = useCallback((ctx: AudioContext, buffer: AudioBuffer, hash: string) => {
    setSignalPath({
      sourceFormat: hash.split('.').pop()?.toUpperCase() ?? null,
      ...
      // bit-perfect output it cannot guarantee.
      bitPerfect: buffer.sampleRate === ctx.sampleRate,
    })
  }, [])

  const scheduleNext = useCallback((ctx: AudioContext, track: ScheduledTrack, when: number) => {
    ...
    node.onended = () => {
      ...
      if (pending && pending.hash === queueRef.current[idx]) {
        ...
        fetchAndDecode(ctx, queueRef.current[idx])
  ...
  const playQueue = useCallback(async (hashes: string[]) => {
    ...
    try {
      const buffer = await fetchAndDecode(ctx, hashes[0])
